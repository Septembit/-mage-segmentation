...
def dataVallist():
        valImage = []
        valLabel = []
        dataFile = "lists/100k/drivable/val_images.txt"
        labelFile = "lists/100k/drivable/val_labels.txt"
        with open(dataFile, "r") as f:
            data = f.readlines()
            for line in data:
                valImage.append(os.path.join("/home/yachao-li/Downloads/bdd100k_images/bdd100k",
                                               line.split("\n")[0]))
        with open(labelFile, "r") as f:
            data = f.readlines()
            for line in data:
                valLabel.append(os.path.join("/home/yachao-li/Downloads/bdd100k_drivable_maps/bdd100k",
                                               line.split("\n")[0]))
        return valImage, valLabel

class dataValloader(data.Dataset):

    def __init__(self):
        self.valList = dataVallist()[0]
        self.valLabel = dataVallist()[1]
        print("# of training samples:", len(self.trainList))

    def __getitem__(self, index):
        img_path_list = self.valList[index]
        label_path_list = self.valLabel[index]
